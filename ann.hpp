...
*/
    ...
    void save_weights(vector<T>& ww, vector<T>& bb) {
        ww.assign(&ww_[0], &ww_[total_ww_size_]);
        bb.assign(&bb_[0], &bb_[total_bb_size_]);
    }
    void restore_weights(vector<T>& ww, vector<T>& bb) {
        for (int b = 0; b < total_bb_size_; ++b) {
            bb_[b] = bb[b];
        }
