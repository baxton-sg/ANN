...
##    Y = np.array([[0,0,0]] * N, dtype=np.float64)

    X = sp.concatenate((ds1, ds2, ds3), axis=0).astype(np.float64)
    Y = sp.concatenate(( [[1,0,0]]*ds1.shape[0], [[0,1,0]]*(ds2.shape[0]), [[0,0,1]]*(ds3.shape[0]) ), axis=0).astype(np.float64)

    N = ds1.shape[0] + ds2.shape[0] + ds3.shape[0]

    indices = np.array(range(N), dtype=int)

    sizes = np.array([2, 10, 3], dtype=np.int32)
    ann = ANN_DLL.ann_create(sizes.ctypes.data, ctypes.c_int(3))
    ...
    for cnt in range(1, 6000):
        ...
        np.random.shuffle(indices)
        tmpIndices = indices[:MBS]

        tmpX = X[tmpIndices].astype(np.float64)
        tmpY = Y[tmpIndices].astype(np.float64)

        ANN_DLL.ann_fit(ctypes.c_void_p(ann), tmpX.ctypes.data, tmpY.ctypes.data, ctypes.c_int(MBS), ctypes.addressof(alpha), ctypes.c_double(0), ctypes.c_int(5))
        alpha.value = .08
